...
interface Props {
  ...
  /** レイアウトごとの余白倍率（1 = 標準, 0 で余白なし） */
  layoutPaddingScales?: Record<string, number>;
  /** Title スライドにブランド名を表示するか */
  showTitleBrand?: boolean;
  brandText?: string;
  /** 文字色 */
...
// ─── デザイン定数（フォールバック） ──────────────────────
const W_DEFAULT   = "#FFFFFF";
const B_DEFAULT   = "#111111";
const Y_DEFAULT   = "#FBCB3E";
const YL  = "#FEF3C0";
const MG_DEFAULT  = "#888888";
const LG  = "#CCCCCC";
...
// ─── テーマ Context ────────────────────────────────────
interface Theme {
  primary: string; sub: string; accent: string; bg: string;
  showBrand: boolean; brandText: string;
  titleScale: number;
  paddingScales: Record<string, number>;
}
const DEFAULT_THEME: Theme = {
  primary: B_DEFAULT, sub: MG_DEFAULT, accent: Y_DEFAULT, bg: W_DEFAULT,
  showBrand: false, brandText: "NotebookLM",
  titleScale: 1,
  paddingScales: {},
};
const ThemeContext = React.createContext<Theme>(DEFAULT_THEME);
const useTheme = () => React.useContext(ThemeContext);
const padFor = (theme: Theme, layout: string, base: number): number =>
  Math.max(0, Math.round(base * (theme.paddingScales[layout] ?? 1)));
...
const Bar: React.FC<{ rf: number; dur: number }> = ({ rf, dur }) => {
  ...
  return (
    <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, height: 4, background: "#E8E8E0" }}>
      <div style={{ height: "100%", width: `${Math.min(rf / Math.max(dur, 1), 1) * 100}%`, background: theme.accent }} />
    </div>
  );
};
...
const YCard: React.FC<{
  ...
  return (
    <div style={{ position: "relative", ...style }}>
      <div style={{ position: "absolute", inset: 0, background: theme.accent, borderRadius: radius, border: `2.5px solid ${theme.primary}`, transform: `translate(${offset}px,${offset}px)` }} />
      <div style={{ position: "relative", background: theme.bg, borderRadius: radius, border: `2.5px solid ${theme.primary}`, padding: "36px 44px", minHeight: minH }}>
        {children}
      </div>
    </div>
  );
    ...
    <AbsoluteFill style={{ background: theme.bg }}>
      ...
      <AbsoluteFill style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", opacity: op }}>
        <Appear rf={rf} fps={fps} from="scale">
          {theme.showBrand && theme.brandText && (
            <div style={{ fontFamily: F, fontSize: 22, fontWeight: 400, color: theme.sub, letterSpacing: "0.2em", marginBottom: 20, textAlign: "center" }}>{theme.brandText}</div>
          )}
          <div style={{ width: lw, height: 2, background: theme.primary, margin: "0 auto 32px", opacity: 0.5 }} />
          {seg.slide_icon && <div style={{ fontSize: 56, textAlign: "center", marginBottom: 12 }}>{seg.slide_icon}</div>}
          <div style={{ fontFamily: F, fontSize: fs, fontWeight: 900, color: theme.primary, textAlign: "center", lineHeight: 1.3 }}>
            {seg.slide_title}
          </div>
          {seg.slide_sub && <div style={{ fontFamily: F, fontSize: Math.round(36 * ss), color: theme.sub, textAlign: "center", marginTop: 24 }}>{seg.slide_sub}</div>}
          <div style={{ width: lw, height: 2, background: theme.primary, margin: "32px auto 0", opacity: 0.5 }} />
        </Appear>
      </AbsoluteFill>
      <Bar rf={rf} dur={dur} />
    </AbsoluteFill>
...
const LayoutQuestion: React.FC<LayoutProps> = ({ seg, rf, dur, fps, ts = 1, ss = 1, its = 1 }) => {
  ...
  const padX = padFor(theme, "question", 120);
  return (
    <AbsoluteFill style={{ background: theme.bg }}>
      <GridBg />
      <div style={{ position: "absolute", top: "50%", left: "50%", transform: `translate(-40%,-50%) scale(${scB})`, fontFamily: F, fontSize: 560, fontWeight: 900, color: theme.accent, opacity: 0.5, lineHeight: 1, userSelect: "none" }}>
        {seg.slide_icon || "？"}
      </div>
      <AbsoluteFill style={{ display: "flex", flexDirection: "column", justifyContent: "center", padding: `0 ${padX}px`, opacity: op }}>
        <Appear rf={rf} fps={fps} delay={4} from="bottom">
          <div style={{ fontFamily: F, fontSize: fs, fontWeight: 900, color: theme.primary, lineHeight: 1.4, maxWidth: 1200 }}>
            {seg.slide_title}
          </div>
        </Appear>
        {seg.slide_sub && (
          <Appear rf={rf} fps={fps} delay={8} from="bottom">
            <div style={{ fontFamily: F, fontSize: Math.max(32, Math.round(fs * 0.5 * ss)), color: theme.sub, marginTop: 20, maxWidth: 1200 }}>
              {seg.slide_sub}
            </div>
          </Appear>
        )}
          ...
          <Appear rf={rf} fps={fps} delay={12} from="left">
            <div style={{ display: "flex", gap: 24, marginTop: 40, flexWrap: "wrap" }}>
              {items.slice(0, 4).map((item, i) => (
                <div key={i} style={{ background: YL, borderRadius: 12, border: `2px solid ${theme.accent}`, padding: "12px 24px" }}>
                  <span style={{ fontFamily: F, fontSize: Math.max(28, Math.round(fs * 0.4 * its)), fontWeight: 700, color: theme.primary }}>{item}</span>
                </div>
              ))}
            </div>
          </Appear>
...
const LayoutSection: React.FC<LayoutProps> = ({ seg, rf, dur, fps, ts = 1, ss = 1 }) => {
  ...
  const fs  = scaledAutoFs(seg.slide_title, 72, 40, ts);
  const padX = padFor(theme, "section", 80);
  return (
    <AbsoluteFill style={{ background: theme.accent }}>
      <GridBg color="transparent" />
      <AbsoluteFill style={{ display: "flex", alignItems: "center", padding: `0 ${padX}px`, gap: 60, opacity: op }}>
        <div style={{ transform: `translateX(${tx}px)` }}>
          ...
          <div style={{ fontFamily: F, fontSize: 300, fontWeight: 900, color: theme.bg, lineHeight: 1, WebkitTextStroke: `6px ${theme.primary}` }}>
            {seg.slide_number || "1"}
          </div>
        </div>
        <div style={{ flex: 1 }}>
          <Appear rf={rf} fps={fps} delay={6} from="right">
            <div style={{ background: theme.bg, borderRadius: 24, border: `3px solid ${theme.primary}`, padding: "48px 56px" }}>
              <div style={{ fontFamily: F, fontSize: fs, fontWeight: 900, color: theme.primary, lineHeight: 1.3 }}>{seg.slide_title}</div>
              {seg.slide_sub && <div style={{ fontFamily: F, fontSize: Math.round(38 * ss), color: theme.sub, marginTop: 16 }}>{seg.slide_sub}</div>}
            </div>
          </Appear>
        </div>
      </AbsoluteFill>
...
const LayoutFeature: React.FC<LayoutProps> = ({ seg, rf, dur, fps, ts = 1, ss = 1, its = 1 }) => {
  ...
  const padY = padFor(theme, "feature", 60);
  const padX = padFor(theme, "feature", 100);
  return (
    <AbsoluteFill style={{ background: theme.bg }}>
      <GridBg />
      <AbsoluteFill style={{ display: "flex", alignItems: "center", justifyContent: "center", padding: `${padY}px ${padX}px`, opacity: op }}>
        <div style={{ transform: `scale(${sc})`, width: "100%", maxWidth: 1300 }}>
          <YCard offset={10} radius={20}>
            ...
            <div style={{ fontFamily: F, fontSize: fs, fontWeight: 900, color: theme.primary, lineHeight: 1.3, marginBottom: 20 }}>
              {seg.slide_title}
            </div>
            {seg.slide_sub && (
              <div style={{ fontFamily: F, fontSize: subFs, color: theme.primary, lineHeight: 1.7 }}>{seg.slide_sub}</div>
            )}
            {seg.slide_items?.map((item, i) => (
              <div key={i} style={{ display: "flex", gap: 12, marginTop: 10 }}>
                <span style={{ color: theme.accent, fontWeight: 900, fontSize: itemFs }}>•</span>
                <span style={{ fontFamily: F, fontSize: itemFs, color: theme.primary }}>{item}</span>
              </div>
            ))}
          </YCard>
        </div>
...
const LayoutSplit: React.FC<LayoutProps> = ({ seg, rf, dur, fps, ts = 1, ss = 1, its = 1 }) => {
  ...
  const padY  = padFor(theme, "split", 60);
  const padX  = padFor(theme, "split", 80);
  return (
    <AbsoluteFill style={{ background: theme.bg }}>
      <GridBg />
      <AbsoluteFill style={{ display: "flex", flexDirection: "column", padding: `${padY}px ${padX}px`, opacity: op }}>
        <Appear rf={rf} fps={fps} from="bottom">
          <div style={{ fontFamily: F, fontSize: hFs, fontWeight: 700, color: theme.primary, marginBottom: 40 }}>
            ...
            <Appear key={i} rf={rf} fps={fps} delay={i * 8} from="bottom">
              <div style={{ flex: 1 }}>
                <YCard minH={280} offset={8} radius={16}>
                  ...
                  <div style={{ fontFamily: F, fontSize: iFs, fontWeight: 700, color: theme.primary, lineHeight: 1.6 }}>{item}</div>
                </YCard>
              </div>
            </Appear>
          ))}
...
const LayoutFlow: React.FC<LayoutProps> = ({ seg, rf, dur, fps, ts = 1, ss = 1, its = 1 }) => {
  ...
  const padY  = padFor(theme, "flow", 50);
  const padX  = padFor(theme, "flow", 80);
  return (
    <AbsoluteFill style={{ background: theme.bg }}>
      <GridBg />
      <AbsoluteFill style={{ display: "flex", flexDirection: "column", padding: `${padY}px ${padX}px`, opacity: op }}>
          ...
          {items.slice(0, 3).map((item, i) => (
            <React.Fragment key={i}>
              <Appear rf={rf} fps={fps} delay={i * 8} from="scale">
                <div style={{ background: theme.bg, borderRadius: 16, border: `2px solid ${theme.primary}`, padding: "28px 32px", flex: 1, minHeight: 160 }}>
                  <div style={{ fontFamily: F, fontSize: iFs, fontWeight: 700, color: theme.primary, lineHeight: 1.5 }}>{item}</div>
                </div>
              </Appear>
              {i < Math.min(items.length, 3) - 1 && (
                <div style={{ color: theme.accent, fontSize: 48, fontWeight: 900, opacity: interpolate(rf, [i * 8 + 14, i * 8 + 22], [0, 1], cl) }}>→</div>
              )}
            </React.Fragment>
          ))}
        </div>
...
const LayoutTimeline: React.FC<LayoutProps> = ({ seg, rf, dur, fps, ts = 1, its = 1 }) => {
  ...
  const descFs  = Math.round(48 * its);
  const padY  = padFor(theme, "timeline", 60);
  const padX  = padFor(theme, "timeline", 80);
  return (
    <AbsoluteFill style={{ background: theme.bg }}>
      <GridBg />
      <AbsoluteFill style={{ display: "flex", flexDirection: "column", padding: `${padY}px ${padX}px`, opacity: op }}>
        <Appear rf={rf} fps={fps} from="bottom">
          <div style={{ fontFamily: F, fontSize: hFs, fontWeight: 700, color: theme.primary, marginBottom: 80 }}>
            {seg.slide_icon && <span style={{ marginRight: 16 }}>{seg.slide_icon}</span>}
            {seg.slide_title}
          </div>
        </Appear>
        <div style={{ position: "relative", height: 240 }}>
          <div style={{ position: "absolute", top: 32, left: 0, width: lw, height: 3, background: theme.accent, borderRadius: 2 }} />
          <div style={{ position: "absolute", top: 19, left: lw - 2, color: theme.accent, fontSize: 30, opacity: interpolate(rf, [30, 38], [0, 1], cl) }}>▶</div>
          {items.slice(0, 4).map((item, i) => {
            ...
            return (
              <React.Fragment key={i}>
                <div style={{ position: "absolute", top: 22, left: `${xPct}%`, transform: "translateX(-50%)", width: 20, height: 20, borderRadius: "50%", background: theme.accent, border: `3px solid ${theme.primary}`, opacity: dotOp }} />
                <Appear rf={rf} fps={fps} delay={i * 6 + 8} from="bottom">
                  <div style={{ position: "absolute", top: 58, left: `${xPct}%`, transform: "translateX(-50%)", textAlign: "center", width: 320 }}>
                    <div style={{ fontFamily: F, fontSize: labelFs, fontWeight: 900, color: theme.primary, border: `2px solid ${theme.primary}`, borderRadius: 8, padding: "6px 16px", display: "inline-block", background: theme.bg }}>{label}</div>
                    <div style={{ fontFamily: F, fontSize: descFs, color: theme.sub, marginTop: 14, lineHeight: 1.4 }}>{desc}</div>
                  </div>
                </Appear>
              </React.Fragment>
            );
...
const LayoutBullets: React.FC<LayoutProps> = ({ seg, rf, dur, fps, ts = 1, ss = 1, its = 1 }) => {
  ...
  const iFs   = scaledAutoFs(items[0] || "", 40, 28, its);
  const padY  = padFor(theme, "bullets", 80);
  const padX  = padFor(theme, "bullets", 100);
  return (
    <AbsoluteFill style={{ background: theme.bg }}>
      <GridBg />
      <AbsoluteFill style={{ display: "flex", flexDirection: "column", padding: `${padY}px ${padX}px`, opacity: op }}>
        <Appear rf={rf} fps={fps} from="bottom">
          <div style={{ fontFamily: F, fontSize: hFs, fontWeight: 900, color: theme.primary, marginBottom: 52, lineHeight: 1.3 }}>
            {seg.slide_icon && <span style={{ marginRight: 16 }}>{seg.slide_icon}</span>}
            {seg.slide_title}
          </div>
        </Appear>
          ...
          {items.slice(0, 4).map((item, i) => (
            <Appear key={i} rf={rf} fps={fps} delay={i * 8 + 6} from="left">
              <div style={{ display: "flex", alignItems: "flex-start", gap: 20 }}>
                <div style={{ width: 12, height: 12, borderRadius: "50%", background: theme.primary, marginTop: 14, flexShrink: 0 }} />
                <div style={{ fontFamily: F, fontSize: iFs, color: theme.primary, lineHeight: 1.7 }}>{item}</div>
              </div>
            </Appear>
          ))}
        </div>
...
const LayoutCards: React.FC<LayoutProps> = ({ seg, rf, dur, fps, ts = 1, ss = 1, its = 1 }) => {
  ...
  const hFs   = scaledAutoFs(seg.slide_title, 52, 32, ts);
  const padY  = padFor(theme, "cards", 60);
  const padX  = padFor(theme, "cards", 80);
  ...
  return (
    <AbsoluteFill style={{ background: theme.bg }}>
      ...
      <AbsoluteFill style={{ display: "flex", flexDirection: "column", padding: `${padY}px ${padX}px`, opacity: op }}>
        <Appear rf={rf} fps={fps} from="bottom">
          <div style={{ fontFamily: F, fontSize: hFs, fontWeight: 900, color: theme.primary, marginBottom: 40, lineHeight: 1.3 }}>
            {seg.slide_icon && <span style={{ marginRight: 16 }}>{seg.slide_icon}</span>}
            {seg.slide_title}
          </div>
        </Appear>
              ...
              <Appear key={i} rf={rf} fps={fps} delay={i * 5} from="scale">
                <div style={{ width: items.length <= 3 ? "calc(33.3% - 20px)" : "calc(33.3% - 20px)", minHeight: 180 }}>
                  <div style={{ background: bg, borderRadius: 20, border: `3px solid ${border}`, padding: "28px 24px", height: "100%" }}>
                    ...
                    <div style={{ fontFamily: F, fontSize: iFs, fontWeight: 700, color: theme.primary, lineHeight: 1.5 }}>{titlePart}</div>
...
const Intro: React.FC<{ frame: number; fps: number }> = ({ frame, fps }) => {
  ...
  return (
    <div style={{ position: "absolute", inset: 0, background: theme.bg, opacity: op, transform: `scale(${sc})` }}>
      <GridBg />
      <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
        <div style={{ width: lw, height: 1.5, background: theme.primary, marginBottom: 48 }} />
        <div style={{ fontFamily: F, fontSize: 28, fontWeight: 300, color: theme.sub, letterSpacing: "0.5em" }}>音　声　記　録</div>
        <div style={{ width: lw, height: 1.5, background: theme.primary, marginTop: 48 }} />
      </div>
    </div>
  );
};
...
const Trans: React.FC<{ rf: number }> = ({ rf }) => {
  ...
  return <div style={{ position: "absolute", top: 0, left: "50%", transform: "translateX(-50%)", width: 2, height: h, background: theme.accent }} />;
};

// ─── メイン ────────────────────────────────────────────
export const SubtitleVideo: React.FC<Props> = ({
  fps, originalAudio, segments,
  titleScale, layoutSubScales, layoutItemScales, layoutPaddingScales,
  showTitleBrand, brandText,
  textColorPrimary, textColorSub, textColorAccent, backgroundColor,
}) => {
  const frame = useCurrentFrame();
  const ms2f  = (ms: number) => Math.round(ms / 1000 * fps);

  const theme: Theme = {
    primary:    textColorPrimary ?? B_DEFAULT,
    sub:        textColorSub     ?? MG_DEFAULT,
    accent:     textColorAccent  ?? Y_DEFAULT,
    bg:         backgroundColor  ?? W_DEFAULT,
    showBrand:  showTitleBrand   ?? false,
    brandText:  brandText        ?? "NotebookLM",
    titleScale: titleScale       ?? 1,
    paddingScales: layoutPaddingScales ?? {},
  };
